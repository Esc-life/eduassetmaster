...
export async function saveMapConfiguration(mapImage: string | null, zones: Location[]) {
    const sheetId = await getUserSheetId();
    if (sheetId === 'NO_SHEET') return { success: false, error: 'Ïä§ÌîÑÎ†àÎìúÏãúÌä∏ IDÍ∞Ä ÏÑ§Ï†ïÎêòÏßÄ ÏïäÏïòÏäµÎãàÎã§.' };
...
export async function saveSoftware(item: any) {
    const sheetId = await getUserSheetId();
    if (sheetId === 'NO_SHEET') return { success: false, error: 'Ïä§ÌîÑÎ†àÎìúÏãúÌä∏ IDÍ∞Ä ÏóÜÏäµÎãàÎã§.' };
    if (isGlobalMockMode && !sheetId) return { success: true };
...
export async function registerBulkDevices(devices: any[]) {
    const sheetId = await getUserSheetId();
    if (sheetId === 'NO_SHEET') return { success: false, error: 'Ïä§ÌîÑÎ†àÎìúÏãúÌä∏Í∞Ä Ïó∞ÎèôÎêòÏßÄ ÏïäÏïòÏäµÎãàÎã§.' };
    if (isGlobalMockMode && !sheetId) return { success: true };
...
export async function updateDevice(deviceId: string, updates: Partial<Device>) {
    const sheetId = await getUserSheetId();
    if (sheetId === 'NO_SHEET') return { success: false, error: 'Ω∫«¡∑πµÂΩ√∆Æ∞° ø¨µøµ«¡ˆ æ æ“Ω¿¥œ¥Ÿ.' };
    if (isGlobalMockMode && !sheetId) return { success: true };
